  ...
  public = list(
    ...
    check.API   = function() {
      ...
      if (tResp$status_code != 200) {
        warning("The API is not responding properly. You should not continue.")
      }
      else {
    ...
    check.login = function() {
      ...
      if (self$isLoggedIn() == TRUE) {
        ...
        testURL = paste0(self$get.site(),"ws/v1/test_subscription")
    ...
    current.students = function() {
      aURL = paste0(self$get.site(),"ws/v1/school/1/student/?pagesize=1000")
      
      ASResp = GET(url=aURL,
                   add_headers(.headers = c(Authorization=paste0("Bearer ",self$get.authBearer()))),
                   accept_xml())
      students = xmlParse(rawToChar(ASResp$content), asText = TRUE)
      return(students)
    },
    
    all.students = function() {
      count = self$count.allStudents()
      LE = ceiling(count/1000)
      
      students = list()
      
      for (page in 1:LE) {
        aURL = paste0(self$get.site(),"ws/v1/district/student/?pagesize=1000&page=", page) 
        
        ASResp = GET(url=aURL,
                     add_headers(.headers = c(Authorization=paste0("Bearer ",self$get.authBearer()))),
                     accept_xml())
        
        students = c(students, XML::xmlToList((xmlParse(rawToChar(ASResp$content)))))
        students = students[-length(students)]
      }
      
      return(students)
      # 
      # aURL = paste0(self$get.site(),"ws/v1/district/student/?pagesize=1000")
      # 
      # ASResp = GET(url=aURL,
      #              add_headers(.headers = c(Authorization=paste0("Bearer ",self$get.authBearer()))),
      #              accept_xml())
      # students = XML::xmlToList(xmlParse(rawToChar(ASResp$content), asText = TRUE))
      # 
      # aURL = paste0(self$get.site(),"ws/v1/district/student/?pagesize=1000&page=2")
      # 
      # ASResp = GET(url=aURL,
      #              add_headers(.headers = c(Authorization=paste0("Bearer ",self$get.authBearer()))),
      #              accept_xml())
      ...
      # return(students[-1001][-1297])
    },
    
    count.allStudents = function() {
      aURL = paste0(self$get.site(),"ws/v1/district/student/count")
      
      ASResp = GET(url=aURL,
                   add_headers(.headers = c(Authorization=paste0("Bearer ",self$get.authBearer()))),
                   accept_xml())
      resp = XML::xmlToList(xmlParse(rawToChar(ASResp$content), asText = TRUE))
      return(as.integer(resp$count))
    },
    
    count.assignments = function() {
      aURL = paste0(self$get.site(), "ws/schema/table/psm_assignmentcategory/count")
